...
lmp.command("fix TB all temp/berendsen %d %d %f" %(Tg,Tg,Tdamp))
lmp.command("fix_modify TB temp CSequ")
lmp.command("run %d" %tequilib)
lmp.command("unfix TB")
lmp.command("fix force all aveforce 1.0 1.0 %f" %(Pdamp))
